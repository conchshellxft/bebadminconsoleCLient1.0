...
export default function ExtraGuideTable({
  page,
  setPage,
  rowsPerPage,
  setRowsPerPage,
  ...
  const deleteExtraGuide = (id: string) => {
    // ADMIN_API.post('admin/configuration/delete-extra-guide', {
    //   extra_guide_id: id,
    // })
    //   .then((res) => {
    //     showSnackAlert('success', 'Extra guide deleted successfully!');
    //     setRefresh((prevState: any) => !prevState);
    //   })
    //   .catch((res) => showSnackAlert('error', 'Failed deleting fpx bank!'));
  };
  return (
    <Box sx={{ width: '100%' }}>
              ...
              {rows?.map((row: any, index: number) => {
                return (
                  ...
                  >
                    ...
                    <TableCell style={{ whiteSpace: 'nowrap' }}>
                      <Typography fontSize={15} maxWidth={400} noWrap={true}>
                        {row.content}
                      </Typography>
                    </TableCell>
                    <TableCell style={{ whiteSpace: 'nowrap' }}>
                      {row._id && (
                        <div style={{ display: 'flex' }}>
                          <Button
                            ...
                            onClick={() => deleteExtraGuide(row._id)}
                          >
                            Delete
                          </Button>
                          <Button
                            onClick={() => {
                              setExtraGuideToUpdate(row);
                              setOpen(true);
                            }}
                            style={{
